...
export default function App({ capability = null }: { capability?: CapabilityIntent | null }) {
  ...
  // Повторный тап по активной вкладке «Расход» на сохранённой записи — тот же переезд к пустой карточке, что и «Новый» в её углу.
  ...
  useEffect(()=>{
    ...
    void getCardQueueStatus(id,controller.signal).then(({pendingCount})=>{
      if(controller.signal.aborted)return
      setQueueRuntime({workspaceId:id,pendingCount})
    }).catch(()=>{/* The review queue is supplemental and must not block the workspace. */})
  ...
  // A Bybit sync or a T-Bank statement can change expenses server-side (declined operations void their expense); pull the
  ...
  const create=async(id:string,name:string,currency:string,displayName?:string)=>{
    setError('')
    ...
    try{
      ...
      await createWorkspaceWithProbe(id,name,currency)
...
if(Math.abs(node.scrollLeft-pagerTarget.current)>1)node.scrollLeft=pagerTarget.current;pagerTarget.current=null}const item=navigationTabs[Math.max(0,Math.min(navigationTabs.length-1,Math.round(node.scrollLeft/node.clientWidth)))];if(item)setTab(item.id)},90)
  ...
  if(state.phase==='guest'||state.phase==='no-workspaces'||state.phase==='known-user-locked')return <><main className="empty-state"><div className="brand-mark">m</div><p className="eyebrow">Общие расходы</p><h1>{state.phase==='known-user-locked'?'Вход истёк':'Общий учёт трат для семьи'}</h1><p>{state.phase==='known-user-locked'?'Откройте ссылку доступа, которую сохраняли, — или начните заново.':'Каждый записывает траты со своего телефона, итог за месяц виден всем. Без регистрации и пароля.'}</p>{state.phase==='known-user-locked'?<>{state.conflictingSession&&<button type="button" className="primary" disabled={!online} onClick={()=>void logoutUnexpected().catch((reason)=>setError(reason instanceof Error?reason.message:'Не удалось выйти'))}>Выйти из другого профиля</button>}<button type="button" className={state.conflictingSession?'danger-link':'primary danger'} disabled={!online||!state.session} onClick={()=>void forgetCurrent()}>Начать заново</button>{(!online||!state.session)&&<p className="management-state" role="status">Начать заново можно, когда появится связь с сервером.</p>}</>:<><button type="button" className="primary" disabled={!online} onClick={()=>setCreateOpen(true)}>Создать пространство</button>{state.phase==='guest'&&session&&!session.authenticated&&session.legacyClaimAvailable&&<button type="button" className="sheet-cancel" onClick={()=>updateState(openLegacyClaim)}>Продолжить с существующими расходами</button>}<p className="sheet-copy">Вас пригласили? Откройте ссылку из сообщения.</p></>}{createOpen&&<CreateWorkspaceSheet existing={Boolean(auth)} onClose={()=>setCreateOpen(false)} onCreate={create}/>} {error&&<p className="form-error" role="alert">{error}</p>}</main>{confirmation}</>
  ...
  return <div className="app-shell" key={workspaceId}>
    ...
    {reviewOpen&&<ReviewOverlay onClose={()=>setReviewOpen(false)}><CardReviewView workspaceId={workspaceId} categories={bootstrap.categories} currencies={bootstrap.currencies} tags={bootstrap.tags??[]} categoryOrder={bootstrap.settings?.categoryOrder} tagOrder={bootstrap.settings?.tagOrder} onTag={(tag)=>setWorkspaceData((data)=>({...data,tags:[tag,...(data.tags??[]).filter((item)=>item.id!==tag.id)]}))} online={serverAvailable} onStatus={({pendingCount})=>updateQueueCount(pendingCount)} pendingCount={reviewCount} active onExpenses={(expenses)=>setWorkspaceData((data)=>({...data,expenses:[...expenses,...data.expenses.filter((item)=>!expenses.some((created)=>created.id===item.id))]}))} onExpensesUndo={(expenseIds)=>setWorkspaceData((data)=>({...data,expenses:data.expenses.filter((item)=>!expenseIds.includes(item.id))}))}/></ReviewOverlay>}
...
export { CardReviewView } from './screens/Review'
