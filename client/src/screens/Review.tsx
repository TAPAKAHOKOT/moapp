...
export function CardReviewView({ workspaceId, categories, currencies, tags=[], onTag=()=>{}, online, onExpenses, onExpensesUndo, onStatus, pendingCount=0, active=true }: {workspaceId:string;categories:Category[];currencies:Currency[];tags?:Tag[];onTag?:(tag:Tag)=>void;online:boolean;onExpenses:(expenses:Expense[])=>void;onExpensesUndo:(expenseIds:string[])=>void;onStatus:(status:{pendingCount:number})=>void;pendingCount?:number;active?:boolean}) {
  ...
  useEffect(()=>{
    ...
    listCardTransactions(workspaceId,controller.signal).then((result)=>{
  ...
  const classify=async(categoryId:string)=>{
    ...
    try{const result=await classifyCardTransaction(workspaceId,transaction.id,categoryId,comment,selectedTagIds);action.expenses=result.expenses;onExpenses(result.expenses);removeCurrent(transaction,result.pendingCount);notify('Расход добавлен',{label:'Отменить',run:()=>void undo(action)});tap(8)}
  ...
  const split=async(amounts:number[])=>{
    ...
    try{
      const result=await splitCardTransaction(workspaceId,transaction.id,amounts)
  ...
  const unsplit=async(expenses:Array<{id:string;version:number}>=[])=>{
    ...
    try{
      const result=await unsplitCardTransaction(workspaceId,transaction.id,expenses)
  ...
  return <><section className="page card-review-page" aria-labelledby="card-review-title">
    <h1 className="sr-only" id="card-review-title">Операции с карт</h1>
    {loading?<p className="management-state" role="status">Загружаем операции…</p>:current?(()=>{const amountText=amountNumber(current.amountMinor,current.currency,currencies);return <>
      ...
      <div className="review-operation">
        <article className="review-merchant">
          <CardMark source={current.source}/><div><h3>{current.merchantName||'Без названия продавца'}</h3><p>{current.type==='atm'?'Снятие наличных':current.merchantCategory||'Покупка'}{current.merchantCity?` · ${[current.merchantCity,current.merchantCountry].filter(Boolean).join(', ')}`:''}</p></div>
        </article>
    ...
    </>})():<div className="review-done"><span>✓</span><h3>Всё разобрано</h3><p>Новые операции появятся после обновления карты или новой выписки.</p></div>}
