...
const expenses: Expense[] = [
  { id: 'monday-food', amountMinor: 1_234, currency: 'RSD', categoryId: 'food', note: 'Обед, кофе', occurredAt: '2026-08-24T10:00:00.000Z', createdAt: '2026-08-24T10:00:00.000Z', updatedAt: '2026-08-24T10:00:00.000Z', version: 1, deletedAt: null },
...
function filters(overrides: Partial<HistoryFilters> = {}): HistoryFilters {
  return { categoryId: '', currency: '', period: 'all', date: '2026-08-30', from: '', to: '', ...overrides }
}
...
describe('history filters', () => {
  ...
  it('restores valid saved preferences and ignores malformed storage', () => {
    ...
    expect(saved).toEqual({ query: 'кофе', categoryId: 'food', currency: 'EUR', period: 'range', date: '2026-08-30', from: '2026-08-01', to: '2026-08-31' })
...
describe('history CSV export', () => {
  it('keeps exact decimal amounts and safely quotes notes', () => {
    const csv = buildHistoryCsv([expenses[0]!], categories, currencies)
    expect(csv).toContain('2026-08-24,12:00,Еда,12.34,RSD,"Обед, кофе",monday-food')
    expect(csv.split('\n')).toHaveLength(2)
  })
})
