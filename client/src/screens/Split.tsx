...
export function SplitSheet({ totalMinor, currency, currencies, busy = false, error = '', onClose, onSubmit }: {
  ...
  return <div className="sheet-backdrop" onMouseDown={() => { if (!busy) onClose() }}>
    ...
    >
      ...
      {/* Кнопка не исчезает на пределе частей и на нулевом остатке: пропадая, она дёргала всю шторку. */}
      <button type="button" className="split-add" disabled={busy || rows.length >= MAX_PARTS || draft.remainder <= 0} onClick={addRow}>Ещё часть</button>
