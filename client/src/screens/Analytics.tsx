...
export function AnalyticsView({ userId, workspaceId, bootstrap, setBootstrap = () => {}, theme, accent = 'sage', online, timeZone = appTimeZone(), blocks, period: savedPeriod, editing = false, onEditScreen = () => {}, onScreensChange = () => {} }: { userId: string; workspaceId: string; bootstrap: Bootstrap; setBootstrap?: React.Dispatch<React.SetStateAction<Bootstrap>>; theme: Theme; accent?: Accent; online: boolean; timeZone?: string
  ...
  const calendarSpan=Math.round((new Date(`${selectedRange.to}T12:00:00Z`).getTime()-new Date(`${calendarStart}T12:00:00Z`).getTime())/86400000)+1
  ...
  const calendarGrid=(small:boolean)=><div className={`calendar-heat${small?' small':''}`} style={{'--heat-color':chart.line} as React.CSSProperties} role="img" aria-label={busiestDay?`Календарь трат: больше всего ${detailDate(busiestDay[0])}, ${formatAnalyticsAmount(busiestDay[1],target)}`:'Календарь трат: в этом периоде пусто'}>
    {!small&&['Пн','Вт','Ср','Чт','Пт','Сб','Вс'].map((day)=><small key={day} aria-hidden="true">{day}</small>)}
    {calendarCells.map((date)=>{
      const amount=calendarAmounts.get(date)??0
      const heat=amount&&calendarMax?.18+.82*amount/calendarMax:0
      const outside=date<from||date>selectedRange.to
      return <span key={date} aria-hidden="true" className={[outside?'outside':'',date===today?'today':'',date>today?'future':'',heat>.55?'hot':''].filter(Boolean).join(' ')||undefined} style={{'--heat':String(heat)} as React.CSSProperties}>{!small&&!outside?Number(date.slice(8)):''}</span>
    })}
  </div>
  ...
  const smallCard=(id:string)=>id==='trend'?<div key="trend" className="chart-card small"><h2>Динамика</h2>{data.convertedCount?<div className="mini-chart"><Suspense fallback={<ChartSkeleton/>}><AnalyticsChart kind="line" compact labels={dayLabels} values={byDay} color={chartColor} fillColor={chart.fill} pointRadius={0} target={target} textColor={chartText} gridColor={chartGrid} maxTicksLimit={7}/></Suspense></div>:<p className="mini-empty">{data.expenseCount?'Нет курса':emptyShort}</p>}</div>
  ...
  return <section ref={sectionRef} className={`page analytics${editing?' arranging':''}`}><div className={`analytics-progress${slowLoading?' on':''}`} aria-hidden="true"/><div className="analytics-fixed" inert={editing}><header className="page-header analytics-title"><div><p className="eyebrow">{focusedName??'Все расходы'}</p><h1>{cachedNumberFormat('ru-RU',{maximumFractionDigits:0}).format(shownTotal)}{hasForeign&&<button type="button" className="rate-info" aria-label="Как посчитана сумма" aria-expanded={rateInfo} onClick={()=>setRateInfo((value)=>!value)}>i</button>}</h1><p className="analytics-comparison">{formatAnalyticsAmount(shownPerDay,target)} в день · {data.expenseCount} {pluralRu(data.expenseCount,['операция','операции','операций'])}</p><p className="analytics-comparison">{comparisonLabel(total,previousTotal,partial,period)}</p></div><button className="currency-choice" onClick={()=>setCurrencySheet(true)}>{target}<ChevronIcon/></button></header>
    ...
    {editing?<AnalyticsBlocksEditor blocks={analyticsBlocks} onChange={(next)=>onScreensChange({analyticsBlocks:toBlockLayout(next)})}/>:<div className="analytics-cards">{analyticsBlocks.shown.map((block)=>isSmall(analyticsBlocks,block.id)?smallCard(block.id):block.id==='trend'?<div key="trend" className="chart-card"><div><h2>Динамика</h2><p>{period==='week'?'Понедельник — воскресенье':'По дням выбранного месяца'}</p></div>{data.convertedCount?<div className="line-chart"><Suspense fallback={<ChartSkeleton/>}><AnalyticsChart kind="line" labels={days.map((d)=>new Date(`${d}T12:00`).toLocaleDateString('ru-RU',period==='week'?{weekday:'short'}:{day:'numeric',month:'short'}))} values={byDay} color={chartColor} fillColor={chart.fill} pointRadius={period==='week'?3:0} target={target} textColor={chartText} gridColor={chartGrid} maxTicksLimit={period==='week'?7:6}/></Suspense></div>:<AnalyticsEmpty>{data.expenseCount?'Нет курса для выбранной валюты':emptyPeriod}</AnalyticsEmpty>}</div>
      ...
      :block.id==='pace'?<div key="pace" className="chart-card"><div><h2>Темп</h2><p>{partial?`Если тратить как сейчас, к концу ${periodEnd}`:'За весь период'}</p></div>{data.expenseCount||previousFull?<><p className="pace-value">{partial?'≈ ':''}{formatAnalyticsAmount(forecast,target)}</p><p className="pace-compare">{previousFull?`${period==='week'?'Прошлая неделя':'Прошлый месяц'} целиком — ${formatAnalyticsAmount(previousFull,target)}`:`${period==='week'?'На прошлой неделе':'В прошлом месяце'} трат не было`}</p></>:<AnalyticsEmpty>{emptyPeriod}</AnalyticsEmpty>}</div>
      :block.id==='top'?<div key="top" className="chart-card"><div><h2>Крупные траты</h2><p>{focusedName?`Только ${focusedName}`:period==='week'?'За неделю':'За месяц'}</p></div>{topExpenses.length?<div className="top-list">{topExpenses.map(({expense,date,value})=><div key={expense.id} className="top-row"><CategoryMark category={categoryOf(expense.categoryId)}/><span><b>{categoryName(expense.categoryId)||'Скрытая категория'}</b><small>{detailDate(date)}{detailCaption(expense)}</small></span><span className="legend-value"><b>{money(expense.amountMinor,expense.currency,bootstrap.currencies)}</b>{expense.currency!==target&&<small>≈ {formatAnalyticsAmount(value,target)}</small>}</span></div>)}</div>:<AnalyticsEmpty>{emptyPeriod}</AnalyticsEmpty>}</div>
